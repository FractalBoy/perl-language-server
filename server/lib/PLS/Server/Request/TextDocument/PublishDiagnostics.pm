...
sub new {
    ...
    my ( undef, $dir, $suffix ) =
      File::Basename::fileparse( $uri->file, qr/\.[^\.]*$/ );
    ...
    my $client_has_version_support =
      $PLS::Server::State::CLIENT_CAPABILITIES->{textDocument}
      {publishDiagnostics}{versionSupport};
    ...
    return Future->wait_all(@futures)->then(
        sub {
            my $current_version =
              PLS::Parser::Document::uri_version( $uri->as_string );
        ...
        stderr  => {
            on_read => sub {
                ...
                while ( $$buffref =~ s/^(.*)\n// ) {
                    ...
                    if ( my ( $error, $file, $line_num, $area ) =
                        $line =~ /^(.+) at (.+?) line (\d+)(, .+)?/ )
                    {
...
sub get_perlcritic_errors {
    ...
    if ( length $PLS::Parser::State::CONFIG->{perlcritic}{perlcriticrc} ) {
        foreach my $workspace_folder (
            @{ PLS::Parser::Index->new->workspace_folders } )
        {
            $profile =
              $PLS::Parser::State::CONFIG->{perlcritic}{perlcriticrc} =~
              s/\$ROOT_PATH/$workspace_folder/r;
        ...
        if ( not length $profile ) {
            ($profile) =
              glob $PLS::Server::State::CONFIG->{perlcritic}{perlcriticrc};
        }
    }
        ...
        push @diagnostics,
          {
            range => {
                start => {
                    ...
                    line      => $violation->line_number - 1,
                    character => $violation->column_number +
                      length( $violation->source ) - 1
                }
            },
...
sub run_podchecker {
    ...
    while ( $errors =~ s/^(.*)\n// ) {
        ...
        if ( my ( $severity, $error, $line_num ) =
            $line =~ /^\**\s*([A-Z]{3,}):\s*(.+) at line (\d+) in file/ )
        {
            push @diagnostics,
              {
                range => {
